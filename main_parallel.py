...
def train_translearner(gpu, opts):
    ...
    # optimizers
    tl_optim = torch.optim.Adam(translearner.parameters(), lr=opts.lr, betas=(0.0, 0.9))
    disc_optim = torch.optim.Adam(disc.parameters(), lr=opts.lr, betas=(0.0, 0.9))
    ...
    while True:
        ...
        #times = []
        for states, actions1, actions2, neg_actions1, neg_actions2 in train_loader:
            it += 1
            start = time.time()
            # prepare data
            states, actions1, actions2, neg_actions1, neg_actions2 = utils.get_data(device, 
                                                                                    states, 
                                                                                    actions1, 
                                                                                    actions2, 
                                                                                    neg_actions1, 
            ...
            gloss_dict, gloss, gout = \
                trainer.generator_trainstep(states, actions1, actions2, warm_up, it=it)
            gtime = time.time() - start

            # Discriminator updates
            start1 = time.time()
            dloss_dict = trainer.discriminator_trainstep(states, actions1, actions2, neg_actions1, 
                                                         neg_actions2, warm_up, gout, it=0)
            ...
            if logging:
                with torch.no_grad():
                    ...
                    loss_str = 'Generator [step %d / %d] ' % (it, train_len)
                    ...
                    loss_str = 'Discriminator [step %d / %d] ' % (it, train_len)
            ...
            del gloss_dict, gloss, gout, states, actions1, actions2, neg_actions1, neg_actions2, dloss_dict
            ...
            if it % opts.eval_iter == 0:
                ...
                while True:
                    ...
                    for states, actions1, actions2, neg_actions1, neg_actions2 in val_loader:
                        # prepare data
                        states, actions1, actions2, neg_actions1, neg_actions2 = utils.get_data(device, 
                                                                                                states, 
                                                                                                actions1, 
                                                                                                ...
                                                                                                neg_actions2)
                        with torch.no_grad():
                            loss_dict, gloss, gout = trainer.generator_trainstep(states, actions1, actions2, warm_up, 
                                                                                train=False, it=it)
                            if logging:
                            ...
                            del loss_dict, gloss, gout, states, actions1, actions2, neg_actions1, neg_actions2
